...
def _build_simple_ovpn_config(
    ovpn_config: str,
    creds_file: str,
    patched_config: str,
) -> bool:
    """
    Build a patched .ovpn that ONLY adds auth-user-pass.
    Let OpenVPN handle ALL routing and DNS normally.
    We protect Cloudflare routes separately.
    """
    log_info("Preparing VPN config...")
    ...
    try:
        ...
        for line in lines:
            ...
            # Replace auth-user-pass with our creds file
            if stripped.startswith('auth-user-pass'):
                filtered_lines.append(f'auth-user-pass {creds_file}')
                continue

            # Remove any route-nopull or pull-filter from previous runs
        ...
        log_success("VPN config prepared.")
        return True

    except Exception as exc:
        log_err(f"Failed to prepare config: {exc}")
        return False


def _protect_cloudflare_before_vpn(supervisor_ref=None) -> None:
    """
    BEFORE OpenVPN connects: add static routes for Cloudflare
    IPs via the original gateway. These routes have no metric
    (most specific route wins) so they survive OpenVPN's
    redirect-gateway directive.
    """
    if not _SAVED_GATEWAY or not _SAVED_INTERFACE:
        log_warn("No saved gateway — cannot protect Cloudflare routes.")
        return

    gw = _SAVED_GATEWAY
    iface = _SAVED_INTERFACE

    log_info(f"Pre-protecting Cloudflare routes via {gw} ({iface})...")
    ...
    # Protect the specific tunnel domain if known
    if supervisor_ref and supervisor_ref.tunnel_url:
        hostname = supervisor_ref.tunnel_url.replace(
            "https://", ""
        ...
        try:
            ...
            for r in results:
                ...
                protected += 1
                log_info(f"  Protected tunnel IP: {ip}")
        except Exception:
            pass

    log_success(f"Pre-protected {protected} Cloudflare route(s).")


def _protect_cloudflare_after_vpn(supervisor_ref=None) -> None:
    """
    AFTER OpenVPN connects: re-apply the same Cloudflare routes.
    OpenVPN may have added routes that override ours, so we
    re-apply them to make sure they stick.
    """
    log_info("Re-protecting Cloudflare routes after VPN connected...")
    _protect_cloudflare_before_vpn(supervisor_ref)


def _verify_vpn_working() -> None:
    """Check that internet works AND traffic goes through VPN."""
    C = Colors
    log_info("Verifying VPN connection...")
    ...
    if dns_ok:
        ...
        log_info("  Attempting DNS fix...")
        # If DNS fails, OpenVPN may not have pushed DNS.
        # Add public DNS as fallback while keeping VPN DNS.
        try:
            ...
            if "1.1.1.1" not in current_dns:
                with open("/etc/resolv.conf", "a") as fh:
                    fh.write("\n# Fallback DNS added by VPS Desktop\n")
                    fh.write("nameserver 1.1.1.1\n")
                    fh.write("nameserver 8.8.8.8\n")
                log_info("  Added fallback DNS servers.")

            # Re-test
            import socket as sock_mod
            result = sock_mod.getaddrinfo("google.com", 80)
            if result:
                log_success("  DNS after fix: ✅ working")
        except Exception:
            log_err("  DNS fix failed.")

    # Test 2: Check public IP (shows VPN IP if working)
    try:
        ...
        log_success(
            f"  Public IP: {C.OKGREEN}{public_ip}{C.ENDC} "
            f"(should be VPN IP)"
        )
    except Exception:
        log_warn("  Could not check public IP (may still be working).")
...
def vpn_connect_with_protection(
    ...
    """
    Connect to OpenVPN while protecting the Cloudflare tunnel:

    1. Add static Cloudflare routes via original gateway (BEFORE VPN)
    2. Start OpenVPN normally (let it do its own routing + DNS)
    3. Re-apply Cloudflare routes (AFTER VPN, in case it overwrote them)
    4. Restart Cloudflare tunnel if it died
    5. Verify internet works through VPN

    This approach lets OpenVPN handle ALL routing and DNS normally,
    we ONLY protect the specific Cloudflare IPs needed for the tunnel.
    """
    C = Colors
    fname = os.path.basename(ovpn_config)
    ovpn_log = "/tmp/openvpn.log"

    # ── 1. Save original network info ────────────────────────────
    save_original_network()

    # ── 2. Protect Cloudflare routes BEFORE VPN connects ─────────
    _protect_cloudflare_before_vpn(supervisor_ref)
    ...
    # ── 4. Build config (ONLY adds auth, nothing else) ───────────
    patched_config = "/tmp/.ovpn_patched.ovpn"
    if not _build_simple_ovpn_config(
        ovpn_config, creds_file, patched_config
    ):
        return False

    # ── 5. Kill existing OpenVPN ─────────────────────────────────
    ...
    # ── 6. Start OpenVPN (normal mode — full tunnel) ─────────────
    ...
    # ── 8. Re-protect Cloudflare routes AFTER VPN is up ──────────
    # Wait a moment for OpenVPN to finish setting up routes
    time.sleep(3)
    _protect_cloudflare_after_vpn(supervisor_ref)

    # ── 9. Restart Cloudflare tunnel if it died ──────────────────
    if supervisor_ref is not None:
        ...
        if not supervisor_ref._is_cloudflared_alive():
            ...
            # Wait for new tunnel URL then protect it
            time.sleep(5)
    ...
    print(f"  VNC    : {C.OKGREEN}✅ Protected{C.ENDC}")
    print(f"  Tunnel : {C.OKGREEN}✅ Protected{C.ENDC}")
    print(f"{C.OKCYAN}{thin}{C.ENDC}\n")
...
def openvpn_switcher(supervisor_ref=None) -> None:
    ...
    def disconnect_vpn() -> None:
        ...
        # Clean up VPN routes — restore original default
        log_info("Restoring original network...")
        if _SAVED_GATEWAY and _SAVED_INTERFACE:
            # Remove any VPN split routes
        ...
        if os.path.exists(backup):
            try:
                ...
                log_success("Original DNS restored.")
            except Exception:
                pass
